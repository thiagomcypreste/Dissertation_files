...
globals
[
  time-step                                                                                                     ; Defines the variation of time (Δt) that simulation occurs.
  number-of-accidents                                                                                           ; Count the number of accidents that occur in the simulation.
  car-length                                                                                                    ; Defines the length of car.

; SPECIFIC CONSUMPTION FACTORS
  b0                                                                                                            ; Idling consumption factor.
  b1                                                                                                            ; Roling resistance factor.
  b2                                                                                                            ; Uphill factor.
  b3                                                                                                            ; Air drag factor.
  b4                                                                                                            ; Inertial force factor.
]

; THE DRIVER CLASS: ECO-DRIVERS AND CONVENTIONAL DRIVERS. ====================================================================================================================================================================
...
[
  ...
  speed-limit                                                                                                   ; The maximum speed the car can reach.
  ...
  instantaneous-consumption                                                                                     ; The specific fuel consumption by car.
  current-emission-co2                                                                                          ; Current emission of CO2 by car.
  emission-co2                                                                                                  ; The total emission of CO2 by car.
]

; SETUP CARS ================================================================================================================================================================================================================
to setup-car
  clear-all                                                                                                     ; Reset/Clear all information in variables and display of simulation.
  reset-ticks                                                                                                   ; Reset/Clear the counter of ticks.
  if seed-on [random-seed 12345]

  set b0 2.5E-4                                                                                                 ; 0.00025 l/s
  set b1 2.4525E-5                                                                                              ; 0.000024525 l/m
  set b2 0                                                                                                      ;
  set b3 3.25E-8                                                                                                ; 0.0000000325 ls^2/m^3
  set b4 1.25E-4                                                                                                ; 0.000125 ls^2/m^2

  set number-of-accidents 0
  set car-length 0.18
  set time-step time-step-length                                                                                ; Set the time-step value, which is the input value on the display.
  setup-road                                                                                                    ; Call the setup-road process, to defines the road on the display.
  create-cars
end

; GO: THE LOOPING PROCESS ===================================================================================================================================================================================================
to go
  ask turtles
  [
    ...
    count-distance                                                                                              ; Procedure to calculate the distance traveled by cars.
  ...
  ;if [speed] of turtles = [speed-limit] of turtles
  ;[stop]

  if count turtles = 0                                                                                          ; If there isn't cars in the highway, stop the simulation.
  [stop]
  tick                                                                                                          ; Ticks counter.
end
...
to decision-making-process
  ...
  [
    ...
    ifelse speed >= speed-limit                                                                                 ; and check your own speed to see if it is equal to or greater than the maximum allowed.
    [
      set speed speed-limit                                                                                     ; If your speed is equal to or greater, then the driver keeps your speed and set your status to 'cruise'.
      set status "cruise"
    ]
...
; THE PROCESS OF DISTANCE TRAVELED ==========================================================================================================================================================================================
to count-distance
  set distance-traveled distance-traveled + (( speed * time-step ) * 100 / 3.6 )
  set distance-traveled-km distance-traveled * 0.001
end
...
to crashing-cars-process
  ...
  [
    set dist-car-ahead precision (((distance car-ahead) - car-length) * (100 / 3.6)) 4                          ; Set the distance value in meters of the car ahead of the driver.
    ...
    [
      show (word "The Car "who " ("breed")" " hit the Car " [who] of car-ahead " (" [breed] of car-ahead ")")   ; Print "Who hit who?"

      ask car-ahead
      [
        die                                                                                                     ; Remove the car in front that was hit.
      ]
...
; THE FUEL CONSUMPTION AND EMISSION PROCESS =================================================================================================================================================================================
...
; THE END OF ROAD PROCESS ===================================================================================================================================================================================================
to end-of-road-and-outputs
  if xcor >= max-pxcor                                                                                          ; The actions to take in the end of highway.
  [
    show hmss-traveling-time
    show (word precision consumption 4 " liters")
    show (word precision distance-traveled-km 4 " kilometers")
    show (word precision (distance-traveled-km / consumption) 4 " kilometers/liters")
    show (word precision (consumption / (ticks-traveling-time * time-step)) 6 " liters/s")
    show (word precision ((consumption / (ticks-traveling-time * time-step)) * 60 * 60 ) 6 " liters/hour")
    show (word precision emission-co2 4 "kg of CO2")


    ;show all-dist-car-ahead
;    if all-dist-car-ahead != []
;    [show (word (precision (min all-dist-car-ahead / 0.035) 4) " meters")]                                     ; Show the minimum distance from the car ahead in the simulation. This distance was converted to meters (1 patch = 0.035 meters).
                                                                                                                ; Show word (precision consumption 4) " grams of Carbon Dioxide emitted"
    die                                                                                                         ; Remove all cars in the end of highway.
  ]

  if show-process                                                                                               ; To analyze if all desired procedures are going correctly.
  [
    if who-behind? = "Eco-driver"
    [if breed = eco-drivers [show ( word decision " | " decision-before " | " status " | " reacting? " | " ticks-to-react " | " dist-car-ahead " | " dist)]]
    if who-behind? = "Conv-driver"
    [if breed = conv-drivers [show ( word decision " | " decision-before " | " status " | " reacting? " | " ticks-to-react " | " dist-car-ahead " | " dist)]]
  ]
end
...
to count-time
  ...
  set hmss-traveling-time (word hours "h " minutes "m " seconds "s " milisec "ms")
end
...
; CREATION OF CARS =======================================================================================================================================================================================================
to create-cars
  create-eco-drivers 1                                                                                          ; The creation process of Eco-driver.
  [
    set shape "car"
    set size 1;car-length
    set color green
    set heading 90

    set speed-limit 1
    set speed-min 0
    set speed eco-initial-speed
    set acceleration (eco-acceleration * 3.6 / 100)
    set deceleration (eco-deceleration * 3.6 / 100)
    set following-dist eco-following-dist
    set reaction-time reaction-time-input
    set reacting? FALSE
    set label-color black
    set all-dist-car-ahead []
  ]

  create-conv-drivers 1                                                                                         ; The creation process of Conventional Driver.
  [
    set shape "car"
    set size 1;car-length
    set color red
...
; ROAD CONFIGURATION =======================================================================================================================================================================================================
to setup-road                                                                                                   ; Configuration of highway. For analysis purposes, the patches were colored and numbered so as to be distinguishable.
    ask patches
  [
    if pycor < 4 and pycor > 0 [set pcolor white]
    if pycor = 1 or pycor = 3
    [
...
1032
81
IMPACT OF DRIVING BEHAVIOR IN THE ROAD TRAFFIC DYNAMICS: AN AGENT-BASED MODELING APPROACH\n\nMODEL OF INTERACTIONS BETWEEN CARS
20
0.0
1
...
"" ""
PENS
"Eco-driver" 1.0 0 -14439633 true "" "if count eco-drivers > 0 \n[plot ([speed] of turtle 0 * (100 / 3.6))]"
"Conv-driver" 1.0 0 -2674135 true "" "if count conv-drivers > 0 \n[plot ([speed] of turtle 1 * (100 / 3.6))]"
...
825
350
Distance of car ahead (m)
[dist-car-ahead] of turtle 0
4
1
11
...
825
395
Speed of Eco-driver (m/s)
([speed] of turtle 0) * (100 / 3.6)
4
1
11
...
1000
395
Speed of Conv. Driver (m/s)
([speed] of turtle 1) * (100 / 3.6)
4
1
11
...
"" ""
PENS
"Eco-driver" 1.0 0 -14439633 true "" "if count eco-drivers > 0 \n[plot [consumption] of turtle 0]"
"Conv-driver" 1.0 0 -2674135 true "" "if count conv-drivers > 0 \n[plot [consumption] of turtle 1]"
...
825
485
Inst. consumption of fuel (L)
[instantaneous-consumption] of turtle 0
10
1
11
...
485
Inst. consumption of fuel (L)
[instantaneous-consumption] of turtle 1
10
1
11
